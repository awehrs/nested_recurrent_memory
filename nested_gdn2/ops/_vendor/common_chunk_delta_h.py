"""Vendored from fla/ops/common/chunk_delta_h.py (MIT), with one addition.

Promotion's gradient reaches level 0 as a contribution to the state gradient at
every firing chunk, not just at the final state. fla accumulates that gradient in
registers across its reverse scan and exposes no way to add to it mid-scan, so
the backward kernel is copied here with a ``dh_ext`` argument: a per-chunk
[B, NT, H, K, V] buffer added into ``b_dh`` before each chunk's store, so the
injection participates in the recurrence instead of being tacked on afterwards.

Everything else is upstream's. Re-sync when bumping flash-linear-attention.
"""
...
@triton.heuristics({
    ...
    'HAS_DH_EXT': lambda args: args['dh_ext'] is not None,
...
def chunk_gated_delta_rule_bwd_kernel_dhu_blockdim64(
    ...
    if HAS_DH_EXT:
        dh_ext += (boh * HV + i_h).to(tl.int64) * K*V
    if USE_GK:
    ...
    for i_t in range(NT - 1, -1, -1):
        ...
        if HAS_DH_EXT:
            b_dh1 += tl.load(
                dh_ext + i_t_int64*HV*K*V + o_k1[:, None] * V + o_v[None, :],
                mask=m_dh1, other=0.0,
            ).to(tl.float32)
        ...
        if K > 64:
            ...
            if HAS_DH_EXT:
                b_dh2 += tl.load(
                    dh_ext + i_t_int64*HV*K*V + o_k2[:, None] * V + o_v[None, :],
                    mask=m_dh2, other=0.0,
                ).to(tl.float32)
        ...
        if K > 128:
            ...
            if HAS_DH_EXT:
                b_dh3 += tl.load(
                    dh_ext + i_t_int64*HV*K*V + o_k3[:, None] * V + o_v[None, :],
                    mask=m_dh3, other=0.0,
                ).to(tl.float32)
        ...
        if K > 192:
            ...
            if HAS_DH_EXT:
                b_dh4 += tl.load(
                    dh_ext + i_t_int64*HV*K*V + o_k4[:, None] * V + o_v[None, :],
                    mask=m_dh4, other=0.0,
                ).to(tl.float32)
...
def chunk_gated_delta_rule_bwd_dhu(
    ...
    if dh_ext is not None and state_v_first:
        raise ValueError("dh_ext assumes the [.., K, V] state layout; pass state_v_first=False")
