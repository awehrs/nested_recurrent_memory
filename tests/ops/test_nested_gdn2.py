...
def _rand_inputs_nested(B, T, H, K, V, L, N_MAX, dtype, *, firing_intervals=None, n_queries_per_level=None, b_scale=1.0, seed=42,):
    """
    ...
    g = F.logsigmoid(torch.randn(B, T, H, L, K, device=device, dtype=torch.float32)).to(dtype)
    b = torch.rand(B, T, H, L, K, dtype=dtype, device=device) * b_scale
    w = torch.rand(B, T, H, L, V, dtype=dtype, device=device)
    ...
    if L > 1:
        ...
        write_projections = torch.randn(L - 1, H, K, V, dtype=dtype, device=device) * (V**-0.5)
        ...
        write_projections = torch.empty(0, H, K, V, dtype=dtype, device=device)
...
def test_naive_recurrent_nested_gdn2_gradients_flow():
    ...
    write_projections = write_projections.detach().requires_grad_(True)
    ...
    assert write_projections.grad is not None
    assert write_projections.grad.abs().sum() > 0, "write_projections got zero gradient"
...
def test_naive_recurrent_nested_gdn_L1_matches_stock_gdn2(B, T, H, K, V, dtype):
    """L=1 with TokenSource must reduce to FLA's stock GDN-2."""
    ...
    # Adapt to nested format (L=1)
    g_nested = g_flat.unsqueeze(-2)
    b_nested = b_flat.unsqueeze(-2)
    w_nested = w_flat.unsqueeze(-2)
    ...
    o_nested, _ = naive_recurrent_nested_gdn2(
        q.unsqueeze(-2), k, v, g_nested, b_nested, w_nested,
        mix_weights=mix_weights,
        query_banks=query_banks,
        write_projections=write_projections,
        n_queries_per_level=n_queries,
